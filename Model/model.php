...
class model{
    ...
    {   
        try {
            ...
            // Si la base de datos no existe, conectar solo al servidor
            if ($e->getCode() == 1049) { // Código de error para "base de datos desconocida"
                $this->db = new PDO( "mysql:host=" . MYSQL_HOST , 'root', '');
                $this->_deploy(); // Crear la base de datos y las tablas
                // Intentar conectar nuevamente a la base de datos
                $this->db = new PDO(
                    "mysql:host=" . MYSQL_HOST . ";dbname=" . MYSQL_DB . ";charset=utf8",
                    MYSQL_USER,
                    MYSQL_PASS
    ...
    private function _deploy() {
        ...
        $this->db->query('CREATE DATABASE IF NOT EXISTS _tp_tienda_de_electronica');
        $this->db->query('USE _tp_tienda_de_electronica');
        ...
        if(count($tables) == 0) {
            ...
            --
            -- Servidor: 127.0.0.1
            -- Tiempo de generación: 11-11-2024 a las 18:51:03
            ...
            (29, 10, 'Kirby Air Ride', '2003-07-11', 8),
            (46, 8, 'Bloodborne', '2015-03-24', 1);
        ...
        $stmt = $this->db->prepare('INSERT INTO usuario (id, email, password) VALUES (?, ?, ?)');
        $stmt->execute([
            1,
            'webadmin@gmail.com',
            '$2a$12$QsNHmaj92oXE5.yDT2JzKOla2A2zTEx1fgOpARMgYM9FEZbedQdJ6'
        ]);    
    }
